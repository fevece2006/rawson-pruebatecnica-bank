  ...
  if (loading && sagas.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-md p-8">
        <div className="text-center text-gray-500">Cargando sagas...</div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-8">
      <div className="flex justify-between items-center mb-6 pb-4 border-b-2 border-gray-100">
        <h2 className="text-2xl font-bold text-gray-800">🔄 Monitor de Sagas</h2>
        <button
          onClick={fetchSagas}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-gradient-to-br from-indigo-500 to-purple-600 text-white p-6 rounded-lg shadow-md">
          <div className="text-sm opacity-90 mb-2">Total Sagas</div>
          <div className="text-3xl font-bold">{stats.total}</div>
        </div>
        <div className="bg-gradient-to-br from-green-500 to-emerald-600 text-white p-6 rounded-lg shadow-md">
          <div className="text-sm opacity-90 mb-2">Completadas</div>
          <div className="text-3xl font-bold">{stats.completed}</div>
        </div>
        <div className="bg-gradient-to-br from-red-500 to-pink-600 text-white p-6 rounded-lg shadow-md">
          <div className="text-sm opacity-90 mb-2">Fallidas</div>
        ...
        <div className="bg-gradient-to-br from-orange-500 to-amber-600 text-white p-6 rounded-lg shadow-md">
          <div className="text-sm opacity-90 mb-2">En Progreso</div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b-2 border-gray-200">
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Saga ID</th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Cuenta Origen</th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Cuenta Destino</th>
              ...
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Creada</th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Actualizada</th>
            </tr>
          </thead>
          <tbody>
            {filteredSagas.map((saga) => {
              ...
              return (
                <tr key={saga.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4">
                    <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${statusInfo.class}`}>
                      {statusInfo.label}
                    </span>
                  </td>
